...
- [uses: 1] Enforcement is per-project + project-reachable: enforce hooks check `.claude/enforce` (project) OR `$HOME/.claude/enforce` (global). Template ships the marker; py-init/ts-init + install.sh --project-local create it. enforce-memory's marker `~/.claude/enforce-memory` IS shipped by install.sh (modules.json markers list — Ph83 corrected the earlier NOT-shipped claim), so it is armed by default on the maintainer machine; consuming projects opt in via project-local `.claude/enforce-memory`. Verify firing (pipe a real event, assert exit 2), never just file presence — a registered-but-dormant hook passes a presence test and ships broken.
