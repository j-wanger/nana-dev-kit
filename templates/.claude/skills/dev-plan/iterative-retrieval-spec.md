...
Companion to dev-plan Step 5. Defines the multi-round knowledge retrieval loop.
...
2. **Round 0 (initial).** Map each concept to articles retrieved in Step 4. Compute coverage = concepts-with-article / M.
...
5. **Pass gaps.** Unfillable gaps (no wiki article exists after all rounds) become Step 8 design questions.
...
Maintain a set of all article slugs read across Step 4 + all rounds. Each round's "up to 3" budget counts only NEW articles not in this set. If a search returns only already-read articles, the "no new articles" exit fires.
