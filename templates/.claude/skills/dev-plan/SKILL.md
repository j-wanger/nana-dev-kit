...
**Dashboard-as-primary (Phase 107):** by default — an interactive session with the cockpit scripts present — the positions are collected ON the served decisioning **cockpit** (a 127.0.0.1 ephemeral `scripts/decision-server.py` serving `docs/dashboard.html`, ingested via the deterministic `scripts/validate-decision-response.py`), with per-option reasoning + consequences laid out for comparison; see `direction-brief.md` → "Step 13 flow — dashboard-as-primary". **AskUserQuestion is the fail-open fallback** — the ledger row stays the sole firing evidence and the gate reverts to AskUserQuestion on every other path (cockpit scripts absent, headless/autonomous, the `.dev-wiki/no-act-from-page` opt-out, or any server/validator/timeout failure). The flip changes the default, never removes the fallback ([[HEU-012]]/Ph82: don't self-brick the gate).
