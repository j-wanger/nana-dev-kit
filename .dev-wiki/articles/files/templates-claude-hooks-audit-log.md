...
data_reads: ["stdin (JSON)", "$CLAUDE_MODEL env var"]
data_writes: [".nana/audit.jsonl"]
---

# templates/.claude/hooks/audit-log.sh

PostToolUse hook that appends a JSONL audit record for every file write operation, capturing timestamp, tool name, file path, and model identity.
...
- Reads `CLAUDE_MODEL` env var for model attribution; defaults to `"unknown"`.
