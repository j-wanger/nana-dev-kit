...
Shell/Markdown/Python scaffolding kit (220+ files: 23 .sh hooks, 120+ skill .md, 22+ template .md, 14 memory_server .py, 4 wiki-index .py, 50 eval scenarios, 4 eval schemas, 4 eval validators, 2 benchmark .py/.md, 3 .json, 2 .txt, 1 .yaml, 2 .yml, 1 .toml, 1 Makefile, 1 VERSION, 1 kit-ci.yml, 1 .gitignore, 1 .patch, 1 delivery-report .py). Runtime: bash + jq (hooks + eval). Scaffolds a 5-layer Python dev harness + TypeScript dev harness + dev-wiki lifecycle + knowledge-wiki pipeline into new/existing projects via two operational modes: `install.sh` (one-time global, module-group architecture with --all/--core-only/--no-python/--no-typescript/--project-local/--dry-run/--status flags) and `make sync-rules` (per-project). 2-gate ceremony model (direction + delivery) with autonomous agent flow between gates. 291 automated tests via `make test` + 50 eval scenarios via `make eval`. LongMemEval-S benchmark: FTS5 recall@5 91.0%, hybrid/turn ~95%. v0.5.0 on GitHub.
...
| tests/ | Automated bash test suite (291 tests, including 3 README accuracy + 6 report staleness + 23 functional verification tests) | helpers.sh, test_*.sh | install.sh, scripts/, templates/, docs/ | stdout (pass/fail) |
...
291 automated tests (6 scripts) + 50 eval scenarios (4 categories). `make test` runs regression tests fail-fast in temp dirs. `make eval` runs scored eval separately (requires jq).
