...
nana-dev-kit/
  ...
  scripts/                             # sync-rules.sh, generate-report.py (~350 lines), generate-workflow.py (~800 lines), eval-runner.sh (~310 lines), generate-delivery-report.py (196 lines), register-settings.py (~120 lines), heuristic-dashboard.py (~70 lines), check-install-drift.sh (Phase 76 — deterministic fail-open installed-copy-drift comparator, bash+jq, reads modules.json)
  tests/                               # 19 scripts, ~430 tests (helpers.sh + test_*.sh; incl. test_hook_firing_coverage.sh [coverage gate], test_long_cadence_hooks.sh, test_tooluse_hooks.sh, test_amplifier_emitter.sh)
  templates/
...
~430 automated tests (19 scripts) + 52 eval scenarios (4 categories) + 25 reasoning eval scenarios. `make test` runs regression tests (incl. test_amplifier_emitter.sh — the Phase-68 ruler's own test). `make eval` runs scored eval (jq); eval/amplifier/ is repo-only measurement infra, NOT part of `make eval`. `make dashboard` shows heuristic counter stats. Reasoning eval: `python3 eval/reasoning/run-eval.py` (subagent-based, non-deterministic, judge v2 exemplar-based; 9 modes including --ablation, --analyze, --selective for LOO trace analysis and heuristic coverage).
