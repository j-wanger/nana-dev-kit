...
nana-dev-kit/
  ...
  tests/                               # 27 scripts, ~500 tests (Phase 88: + test_check_tests_were_run.sh paired smoke; Phase 84: + test_eval_hermeticity.sh, test_fixture_provenance.sh, test_lifecycle_hooks_firing.sh) (helpers.sh + test_*.sh; incl. test_hook_firing_coverage.sh [coverage gate], test_long_cadence_hooks.sh, test_tooluse_hooks.sh, test_amplifier_emitter.sh, test_assumption_ledger.sh [Phase 81], test_manifest_freshness.sh + test_scripts_smoke.sh [Phase 82])
  templates/
    AGENTS.md, AGENTS-ts.md, pyproject.toml, .pre-commit-config.yaml
    .claude/
      hooks/                           # 17 lifecycle hooks (session-start, pre-compact, post-commit, audit-log, enforce-spec, enforce-loop, enforce-memory, etc.; detect-loop CUT Phase 88, 75b48af)
...
|-------------|-----------|---------|
| install.sh | `bash install.sh [--all\|--core-only\|--no-python\|--no-typescript\|--project-local\|--dry-run\|--status]` | Module-group global install: core (rules + memory), python (py-init + spec), typescript (ts-init), dev-wiki (6 skill dirs), knowledge-wiki (11 skill dirs). --project-local installs all 16 project-scoped hooks. --status shows runtime inventory. |
