...
Shell/Markdown/Python scaffolding kit (55+ files: 12 .sh, 18 .md, 14 .py, 2 .json, 2 .txt, 1 .yaml, 1 .yml, 1 .toml, 1 Makefile, 1 VERSION, 1 kit-ci.yml, 1 .gitignore). Runtime: bash + python3. Scaffolds a 5-layer Python dev harness into new/existing projects via two operational modes: `install.sh` (one-time global, includes memory MCP server with venv bootstrap + /spec skill) and `make sync-rules` (per-project). 55 automated tests via `make test`. v0.2.0 on GitHub.

## Directory Layout

nana-dev-kit/
  install.sh                           # Global installer (4 files + memory_server + MCP + venv)
...
|-------------|-----------|---------|
| install.sh | `bash install.sh` | One-time global install: copies py-init + spec skills + nana-soul + nana-personal rules + memory_server to ~/.claude/, registers MCP server, stores kit path |
...
|--------|---------|-----------|--------|---------|
| root | Global installer and project targets | install.sh, Makefile, VERSION | templates/.claude/ | ~/.claude/skills/ (py-init + spec), ~/.claude/rules/ (soul + personal), ~/.claude/memory_server/, ~/.claude/memory_server/.venv/ |
...
| tests/ | Automated bash test suite (55 tests) | helpers.sh, test_*.sh | install.sh, scripts/, templates/ | stdout (pass/fail) |
...
|--------|-------------|---------------|----------|-------|
| install.sh | templates/.claude/skills/py-init/SKILL.md, templates/.claude/skills/spec/, templates/.claude/rules/nana-soul.md, templates/.claude/rules/nana-personal.md, memory_server/ | ~/.claude/skills/py-init/SKILL.md, ~/.claude/skills/spec/, ~/.claude/rules/nana-soul.md, ~/.claude/rules/nana-personal.md, ~/.claude/.nana-dev-kit-path, ~/.claude/memory_server/, ~/.claude/settings.json | -- | One-time global install + MCP registration |
