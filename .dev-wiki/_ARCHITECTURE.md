...
> Last updated: 2026-06-14 by /dev-debrief (Phase 91 — Consuming-Project Memory E2E + Assumption-Gate Forcing Function: NEW hook `enforce-assumption-gate.sh` [18th project hook, PreToolUse Write|Edit|MultiEdit, mirrors enforce-spec, blocks impl writes when the active phase has no valid assumption-ledger block via `--gate`]; register-settings.py `cmd_mcp` now emits an `env` dict + modules.json mcp block gains `env` {PYTHONPATH:$HOME/.claude} — fixes the memory MCP consumer break (`-m memory_server` resolved only from the package-bearing kit cwd); tests 28 scripts / eval 50 stable; broader file inventory may be stale, run /dev-scan to refresh)
