...
nana-dev-kit/
  ...
  companion/                           # GITIGNORED, local-only research apparatus (Phase 97+) — NEVER shipped/installed. Analogous to the eval/ measurement programs but gitignored. research/ (Phase 97 Frontier Positioning Sweep, rung A of the de-risking ladder): pre-registration.md (the FROZEN decision instrument; serves the spec function for this private phase), .frozen (SHA256 freeze-attestation seal — git ancestry can't guard an untracked file), sweep-findings.md + convergence-map.md (cited evidence + rubric), verdict.md. `git ls-files companion/` is empty by invariant (T1 guard; `companion/` is in .gitignore)
