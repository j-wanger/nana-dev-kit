...
> Last updated: 2026-05-29 by /dev-debrief (Phase 62 — test-suite count synced to 13 scripts; wk-prune.sh now the deterministic hot-cache curator; broader file inventory may be stale, run /dev-scan to refresh)
...
nana-dev-kit/
  ...
  tests/                               # 13 scripts, ~350 tests (helpers.sh + test_*.sh; incl. test_step_numbering.sh, test_working_knowledge_curation.sh)
  templates/
...
| tests/ | Automated bash test suite (~350 tests, 13 scripts, incl. 3 README accuracy + 6 report staleness + 23 functional verification + ~10 register-settings.py + 2 companion validation + 9 heuristic/eval + 11 heuristic evolution + cognitive-readiness firing + 6 step-numbering-continuity + 11 working-knowledge-curation invariant assertions) | helpers.sh, test_*.sh | install.sh, scripts/, templates/, docs/, modules.json | stdout (pass/fail) |
...
~350 automated tests (13 scripts) + 52 eval scenarios (4 categories) + 25 reasoning eval scenarios. `make test` runs regression tests. `make eval` runs scored eval (jq). `make dashboard` shows heuristic counter stats. Reasoning eval: `python3 eval/reasoning/run-eval.py` (subagent-based, non-deterministic, judge v2 exemplar-based; 9 modes including --ablation, --analyze, --selective for LOO trace analysis and heuristic coverage).
