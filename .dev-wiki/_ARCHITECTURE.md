...
nana-dev-kit/
  ...
  tests/                               # 18 scripts, ~420 tests (helpers.sh + test_*.sh; incl. test_hook_firing_coverage.sh [coverage gate], test_long_cadence_hooks.sh, test_tooluse_hooks.sh)
  templates/
...
| tests/ | Automated bash test suite (~420 tests, 18 scripts, incl. 3 README accuracy + 6 report staleness + 23 functional verification + ~10 register-settings.py + 2 companion validation + 11 working-knowledge-curation invariant + hook firing-coverage gate [test_hook_firing_coverage.sh] + long-cadence + tooluse firing-test assertions) | helpers.sh, test_*.sh | install.sh, scripts/, templates/, docs/, modules.json | stdout (pass/fail) |
...
~420 automated tests (18 scripts) + 52 eval scenarios (4 categories) + 25 reasoning eval scenarios. `make test` runs regression tests. `make eval` runs scored eval (jq). `make dashboard` shows heuristic counter stats. Reasoning eval: `python3 eval/reasoning/run-eval.py` (subagent-based, non-deterministic, judge v2 exemplar-based; 9 modes including --ablation, --analyze, --selective for LOO trace analysis and heuristic coverage).
