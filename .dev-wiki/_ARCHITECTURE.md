...
> Last updated: 2026-05-30 by /dev-debrief (Phase 71 — added repo-only eval/amplifier/retention-screen/ subtree [deterministic cross-boundary retention-headroom screen: check.sh clones anchor-screen/'s run_check/aggregate/verify_pins byte-VERBATIM, only diff_verdict new — differential OFF×ON; pre-registration + verdicts; NO LLM in scoring; NOT in install.sh / make test / make eval]; sibling to the Phase-70 anchor-screen/ subtree; test count UNCHANGED at 19 scripts / 52 eval scenarios; broader file inventory may be stale, run /dev-scan to refresh)
