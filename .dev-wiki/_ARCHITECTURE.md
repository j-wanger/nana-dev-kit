...
Shell/Markdown/Python scaffolding kit (260+ files: 23 .sh hooks, 120+ skill .md, 22+ template .md, 14 memory_server .py, 4 wiki-index .py, 50 eval scenarios, 20 reasoning scenarios, 4 eval schemas, 4 eval validators, 15 heuristic .md (10 HEU + 5 IRON), 2 benchmark .py/.md, 5 .json, 2 .txt, 1 .yaml, 2 .yml, 1 .toml, 1 Makefile, 1 VERSION, 1 kit-ci.yml, 1 .gitignore, 1 .patch, 2 scripts .py, 1 reasoning eval .py, 7 ablation trace .json + attribution-matrix.json + selection-criteria.md). Runtime: bash + jq (hooks + eval). Scaffolds a 5-layer Python dev harness + TypeScript dev harness + dev-wiki lifecycle + knowledge-wiki pipeline into new/existing projects via two operational modes: `install.sh` (one-time global, reads modules.json via jq, delegates JSON merges to scripts/register-settings.py, with --all/--core-only/--no-python/--no-typescript/--project-local/--dry-run/--status flags) and `make sync-rules` (per-project). 2-gate ceremony model (direction + delivery) with autonomous agent flow between gates. ~325 automated tests via `make test` + 50 eval scenarios via `make eval` + 20 reasoning eval scenarios. LongMemEval-S benchmark: FTS5 recall@5 91.0%, hybrid/turn ~95%. v0.5.0 on GitHub.
...
nana-dev-kit/
  ...
  wiki/                                # Knowledge wiki (cognitive-patterns): heuristics/ (SCHEMA.md + 10 HEU-*.md + 5 IRON-*.md), articles/, inbox/
...
| wiki/ | Knowledge wiki (cognitive-patterns): heuristic store with 10 seed heuristics + 5 IRON RULES, structured SCHEMA.md (status enum includes iron) | heuristics/SCHEMA.md, heuristics/HEU-*.md, heuristics/IRON-*.md, schema.md, index.md | .dev-wiki/articles/decisions/, working-knowledge.md (seed source) | Heuristic articles for session-start injection + reasoning eval |
| eval/ | Eval harness: benchmark corpus + scoring (50 scenarios) + harness effectiveness comparison + reasoning eval (20 scenarios, judge v2 exemplar-based, IRON RULES delta + self-dialogue + ablation traces) | corpus/*/scenario.json, schemas/*.json, validators/*.sh, comparison/, reasoning/ (judges/, corpus/, baseline/, with-iron-rules/, with-self-dialogue-inline/, with-self-dialogue-subagent/, traces/) | templates/.claude/hooks/*, skill outputs, wiki/heuristics/ (reasoning eval) | Scored eval report (text), comparison results, reasoning baseline + IRON RULES delta + self-dialogue results + ablation attribution matrix |
...
| tests/ | Automated bash test suite (~320 tests, including 3 README accuracy + 6 report staleness + 23 functional verification + ~10 register-settings.py tests + 2 companion validation + 9 heuristic/eval assertions) | helpers.sh, test_*.sh | install.sh, scripts/, templates/, docs/, modules.json | stdout (pass/fail) |
...
~325 automated tests (7 scripts) + 50 eval scenarios (4 categories) + 20 reasoning eval scenarios. `make test` runs regression tests. `make eval` runs scored eval (jq). Reasoning eval: `python3 eval/reasoning/run-eval.py` (subagent-based, non-deterministic, judge v2 exemplar-based; --ablation and --analyze modes for LOO trace analysis).
