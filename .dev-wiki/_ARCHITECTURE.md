...
> Last updated: 2026-06-10 by /dev-debrief (Phase 87 — Ceremony Stage-2 Episode Contrast: NEW `eval/ceremony-lift/stage2/` frozen apparatus [drivability spike record, execution-protocol addendum, amendments/ pre-unblinding deviation mechanism, 5 checkers + 18 seeded controls, instrument/arm/ship/cost/results records, run-exit-criteria.sh 8/8 RUN-STATUS LIVE]; evidence/verdicts-only — NO kit component touched [kit-commit embargo vs pinned base 6728e2f]; make-test count unchanged at 26; substrate edge-screener shipped a6effcb [394 tests / 94.58% cov]; broader file inventory may be stale, run /dev-scan to refresh)

## Project Shape

Shell/Markdown/Python scaffolding kit (260+ files: 21 .sh hooks (18 + 3 session-start.d), 120+ skill .md, 22+ template .md, 14 memory_server .py, 4 wiki-index .py, 52 eval scenarios, 25 reasoning scenarios, 4 eval schemas, 4 eval validators, 16 heuristic .md (11 HEU + 5 IRON), 2 benchmark .py/.md, 5 .json, 2 .txt, 1 .yaml, 2 .yml, 1 .toml, 1 Makefile, 1 VERSION, 1 kit-ci.yml, 1 .gitignore, 1 .patch, 3 scripts .py (incl. heuristic-dashboard.py), 1 reasoning eval .py, 7 ablation trace .json + attribution-matrix.json + selection-criteria.md). Runtime: bash + jq (hooks + eval). Scaffolds a 5-layer Python dev harness + TypeScript dev harness + dev-wiki lifecycle + knowledge-wiki pipeline into new/existing projects via two operational modes: `install.sh` (one-time global, reads modules.json via jq, delegates JSON merges to scripts/register-settings.py, with --all/--core-only/--no-python/--no-typescript/--project-local/--dry-run/--status flags) and `make sync-rules` (per-project). 2-gate ceremony model (direction + delivery; the direction gate is now the assumption-approval gate — positions REPLACE approach-approval, Phase 81) with autonomous agent flow between gates. ~500 automated tests via `make test` (25 scripts) + 52 eval scenarios via `make eval` + 25 reasoning eval scenarios. LongMemEval-S benchmark: FTS5 recall@5 91.0%, hybrid/turn ~95%. v0.5.0 on GitHub.
...
nana-dev-kit/
  ...
  tests/                               # 25 scripts, ~500 tests (Phase 84: + test_eval_hermeticity.sh, test_fixture_provenance.sh, test_lifecycle_hooks_firing.sh) (helpers.sh + test_*.sh; incl. test_hook_firing_coverage.sh [coverage gate], test_long_cadence_hooks.sh, test_tooluse_hooks.sh, test_amplifier_emitter.sh, test_assumption_ledger.sh [Phase 81], test_manifest_freshness.sh + test_scripts_smoke.sh [Phase 82])
  templates/
    AGENTS.md, AGENTS-ts.md, pyproject.toml, .pre-commit-config.yaml
    .claude/
      hooks/                           # 18 lifecycle hooks (session-start, pre-compact, post-commit, audit-log, enforce-spec, enforce-loop, enforce-memory, detect-loop, etc.)
...
| scripts/eval-runner.sh | `make eval` | Runs eval corpus (52 scenarios in 4 categories), produces scored report. Requires jq. |
