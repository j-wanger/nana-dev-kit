#!/usr/bin/env bash
# check.sh — deterministic named-clause checker for the single-decision anchor-headroom screen.
#
# NO LLM, NO embedding, NO judge. A model OUTPUT file is classified against an anchor's
# PRE-REGISTERED clauses (a .check file). This is the deterministic core that replaces an
# LLM-as-judge — see ../VALID-MEASUREMENT.md and the Phase-70 spec for why consensus-by-clause
# (not OR-of-failures, not a fuzzy score) is the guard against false HAS-HEADROOM.
#
...
#   check.sh --selftest                            → exits 0 iff planted PASS/FAIL/HEDGED classify + pins verify
...
case "${1:---selftest}" in
  ...
  --selftest)    selftest;;
  *) echo "usage: check.sh --run|--aggregate|--verify-pins|--selftest ..." >&2; exit 2;;
esac
